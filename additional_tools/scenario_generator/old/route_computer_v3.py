...
def routeComputer(density, repetition, nacInst, nacTotal, spawnInterval, scenarioDuration, \
    ...
    for i in range(int(nacTotal)):
        ...
        tempDestLat, tempDestLon = qdrpos(tempOriginLat, tempOriginLon, heading[i], distance[i])
        ...
        originLat[i] = tempOriginLat
        originLon[i] = tempOriginLon
        destLat[i]   = tempDestLat
        destLon[i]   = tempDestLon
