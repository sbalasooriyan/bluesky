...
def confplot(colnames,data,inst_labels,ylabel,outDir,outName):
...
#    lines.append("                     ymin=1,\n")
    ...
    lines.append("                     tick label style={font=\\footnotesize,major tick length=0},\n")
...
def boxplot(colnames,data,inst_labels,ylabel,outDir,outName,fill=1):
    if len(inst_labels) == 3:
        colopt = {2: "bluevec", 1: "greenvec", 0: "redvec"}
        if fill:
    ...
    lines.append("                     xminorgrids]\n")
    for i in range(len(inst_labels)):
        lines.append("            \\addlegendimage{legend image code/.code={\\draw[%s%s] (-0.02,0.075) rectangle (0.02,0.105);\\draw[%s] (-0.04,0.075) -- (-0.04,0.105);\\draw[%s] (0.04,0.075) -- (0.04,0.105);\\draw[%s] (0.0,0.075) -- (0.0,0.105);\\draw[%s] (-0.04,0.09) -- (-0.02,0.09);\\draw[%s] (0.04,0.09) -- (0.02,0.09);}}\n" %(colopt[i],filler[i],colopt[i],colopt[i],colopt[i],colopt[i],colopt[i]))
    for i in range(len(colnames)):
        for j in range(len(data[i])):
...
# Settings
pdFileName    = 'final_10_12345.pickle'
...
#            box_cfl[k].append(entry)
    ...
    confplot(colnames,dat_cfl,inst_labels,ylabel_cfl,outDir,out_cfl)
    confplot(colnames[:-1],dat_cfltime,inst_labels,ylabel_cfltime,outDir,out_cfltime)
    confplot(colnames[:-1],dat_timeincfl,inst_labels,ylabel_timeincfl,outDir,out_timeincfl)
    confplot(colnames[:-1],dat_los,inst_labels,ylabel_los,outDir,out_los)
    confplot(colnames[:-1],dat_lostime,inst_labels,ylabel_lostime,outDir,out_lostime)
    confplot(colnames[:-1],dat_timeinlos,inst_labels,ylabel_timeinlos,outDir,out_timeinlos)
    confplot(colnames[:-1],dat_dep,inst_labels,ylabel_dep,outDir,out_dep)
    confplot(colnames[:-1],dat_ipr,inst_labels,ylabel_ipr,outDir,out_ipr)
    confplot(colnames[:-1],dat_sev,inst_labels,ylabel_sev,outDir,out_sev)
    confplot(colnames[:-1],dat_mcfl,inst_labels,ylabel_mcfl,outDir,out_mcfl)
    confplot(colnames,dat_time,inst_labels,ylabel_time,outDir,out_time)
    confplot(colnames,dat_work,inst_labels,ylabel_work,outDir,out_work)
    confplot(colnames,dat_density,inst_labels,ylabel_density,outDir,out_density)
    confplot(colnames,dat_dist,inst_labels,ylabel_dist,outDir,out_dist)
    confplot(colnames[:-1],dat_timee,inst_labels,ylabel_timee,outDir,out_timee)
    confplot(colnames[:-1],dat_worke,inst_labels,ylabel_worke,outDir,out_worke)
    confplot(colnames[:-1],dat_densitye,inst_labels,ylabel_densitye,outDir,out_densitye)
    confplot(colnames[:-1],dat_diste,inst_labels,ylabel_diste,outDir,out_diste)
    confplot(colnames[:-1],dat_inscmpcfl,inst_labels[1:],ylabel_inscmpcfl,outDir,out_inscmpcfl)
    confplot(colnames[:-1],dat_inscmplos,inst_labels[1:],ylabel_inscmplos,outDir,out_inscmplos)
    
    boxplot(colnames,dat_cfl,inst_labels,ylabel_cfl,outDir,out_cfl,1)
    boxplot(colnames[:-1],dat_cfltime,inst_labels,ylabel_cfltime,outDir,out_cfltime)
    boxplot(colnames[:-1],dat_timeincfl,inst_labels,ylabel_timeincfl,outDir,out_timeincfl)
    boxplot(colnames[:-1],dat_los,inst_labels,ylabel_los,outDir,out_los)
    boxplot(colnames[:-1],dat_lostime,inst_labels,ylabel_lostime,outDir,out_lostime)
    boxplot(colnames[:-1],dat_timeinlos,inst_labels,ylabel_timeinlos,outDir,out_timeinlos)
    boxplot(colnames[:-1],dat_dep,inst_labels,ylabel_dep,outDir,out_dep)
    boxplot(colnames[:-1],dat_ipr,inst_labels,ylabel_ipr,outDir,out_ipr)
    boxplot(colnames[:-1],dat_sev,inst_labels,ylabel_sev,outDir,out_sev)
    boxplot(colnames[:-1],dat_mcfl,inst_labels,ylabel_mcfl,outDir,out_mcfl)
    boxplot(colnames,dat_time,inst_labels,ylabel_time,outDir,out_time)
    boxplot(colnames,dat_work,inst_labels,ylabel_work,outDir,out_work)
    boxplot(colnames,dat_density,inst_labels,ylabel_density,outDir,out_density)
    boxplot(colnames,dat_dist,inst_labels,ylabel_dist,outDir,out_dist)
    boxplot(colnames[:-1],dat_timee,inst_labels,ylabel_timee,outDir,out_timee)
    boxplot(colnames[:-1],dat_worke,inst_labels,ylabel_worke,outDir,out_worke)
    boxplot(colnames[:-1],dat_densitye,inst_labels,ylabel_densitye,outDir,out_densitye)
    boxplot(colnames[:-1],dat_diste,inst_labels,ylabel_diste,outDir,out_diste)
    boxplot(colnames[:-1],dat_inscmpcfl,inst_labels[1:],ylabel_inscmpcfl,outDir,out_inscmpcfl)
    boxplot(colnames[:-1],dat_inscmplos,inst_labels[1:],ylabel_inscmplos,outDir,out_inscmplos)
